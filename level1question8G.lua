...
local function buttonOnRelease(event)
	local button = event.target.id
		...
		elseif button == "choice1" then
			...
			if curLvl == 3 then storyboard.purgeScene( "level1question1G", false ); storyboard.gotoScene( "level1scene3G", "fade", 200 ) 
			elseif curLvl == 5 then storyboard.purgeScene( "level1question1G", false ); storyboard.gotoScene( "level1scene6G", "fade", 200 ) end
		elseif button == "choice2" then
			score.add(20); score.save(); gameSettings.lev 
			gameSettings.levels[curLvl].score = score.get(); loadsave.saveTable(gameSettings, "myTable.json", system.DocumentsDirectory); loadsave.printTable(gameSettings)
			if curLvl == 3 then storyboard.purgeScene( "level1question1G", false ); storyboard.gotoScene( "level1scene6G", "fade", 200 ) 
			elseif curLvl == 5 then storyboard.purgeScene( "level1question1G", false ); storyboard.gotoScene( "levelfailedG", "fade", 200 ) end
...
function scene:createScene( event )
	...
	if gameSettings.lang == "english" then
		...
		if curLvl == 3 then text = {"You will get out of the game.", "You will reply with lots of curse before getting out of the game.", "You will ignore him and continue playing."}
		elseif curLvl == 5 then text = {"", " ", " "}
		end
	elseif gameSettings.lang == "tagalog" then
		textQuest.text = "Anong gagawin mo?"
		if curLvl == 3 then text = {"Huwag nang maglaro/Umalis ka na sa laro.", "Sasagot ka ng mas marami pang mura bago ka umalis.", "Hayaan lamang sila at ipagpatuloy ang paglalaro"}
		elseif curLvl == 5 then text = {"", " ", " "}
		end
	elseif gameSettings.lang == "bicol" then
		textQuest.text = "Anong gigibuhon mo?"
		if curLvl == 3 then text = {"Dae ka na magakawat at maali ka na", "Mumudahun mo sinda bago ka magali ", "Bayaan sana sinda at tigpadagos ang pagkawat."}
		elseif curLvl == 5 then text = {"", " ", " "}
		end
	end
	...
	local images = {}
	if curLvl == 3 then
		images = {"images/"..gameSettings.lang.."/"..gameSettings.character.."/level"..curLvl.."/scene3b.jpg", "images/"..gameSettings.lang.."/"..gameSettings.character.."/level"..curLvl.."/scene4b.jpg", "images/"..gameSettings.lang.."/"..gameSettings.character.."/level"..curLvl.."/scene1a.jpg"}
	...
	local function choice1fn()		
		...
		animation = transition.to(text,{ time=500,  x=_W/2 + 10, y = _H/2 + _H/3 , xScale=2, yScale=2, transition=fadeIn,customProperty=1000,onComplete=after})
			transition.to(text,{time=500,delay=2500, x=_W/4, y = _H/2 + 10, xScale=1, yScale=1, transition=easing.inQuad})	
	...
	local function choice3fn()
		...
		local text = display.newText( text[table.indexOf( images, randomImages[3] )], 270, 10, "riffic", 12 ); text:setFillColor(0,0,0)
		...
		animation = transition.to(text,{ time=500,  x=_W/2 + 10, y = _H/2 + _H/3, xScale=2, yScale=2, transition=fadeIn,customProperty=1000,onComplete=after})
			transition.to(text,{time=500,delay=2500, x=_W/2, y = _H - _H/20, xScale=1, yScale=1, transition=easing.inQuad})	
		tmr = timer.performWithDelay(1000,function(e)  transition.cancel(animation); animation = nil; end,1); 
